-- Privacy-safe community stats view
-- Returns aggregated statistics ONLY when a strain has sufficient sessions
-- to prevent inference attacks and protect individual user data
--
-- PRIVACY THRESHOLD: 10 sessions minimum
-- This threshold exists to:
-- 1. Prevent identifying individual users from small sample sizes
-- 2. Ensure statistical meaningfulness of aggregated data
-- 3. Protect user privacy by obscuring individual contributions

DROP VIEW IF EXISTS public.strain_community_stats;

CREATE VIEW public.strain_community_stats AS
SELECT 
  strain_id,
